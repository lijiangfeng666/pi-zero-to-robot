...
- [第三步：安装依赖](#第三步安装依赖)
- [第四步：部署代码](#第四步部署代码)
- [第五步：申请 API Key](#第五步申请-api-key)
- [第六步：测试语音](#第六步测试语音)
- [第七步：部署音乐服务](#第七步部署音乐服务)
- [第八步：启动机器人](#第八步启动机器人)
- [验收清单](#验收清单)
- [日常使用](#日常使用)
- [故障排查](#故障排查)
...
| 硬件 | 规格 | 参考价 | 购买建议 |
|------|------|--------|---------|
| **树莓派 4B** | 2GB/4GB/8GB 都行 | 200-400元 | 别买错成 3B 或 Zero |
| **MicroSD 卡** | **64GB**，Class 10 | 50-70元 | 闪迪或三星 |
| **USB 读卡器** | 电脑没 SD 槽就要买 | 10-20元 | 大部分笔记本自带 |
| **电源** | USB-C，**5V/9V/12V/24V 多电压充电器** | 30-50元 | 3C认证，搜"树莓派多电压充电器" |
| **USB 摄像头麦克风一体机** | 免驱，自带麦克风阵列 | 50-100元 | 搜"USB 摄像头麦克风免驱"，同时提供音频输入和视觉输入 |
| **USB 音箱** | **Y57 小扩音器**或类似免驱 USB 音箱 | 30-80元 | 搜"USB 小音箱免驱"，即插即用 |

> **总预算：约 350-660 元**。全部淘宝可买到。

### 麦克风和音响注意事项

```
✅ 推荐：USB 摄像头麦克风一体机（同时解决视频+音频输入）+ USB 小音箱（即插即用）
❌ 不推荐：蓝牙音箱（连接复杂、延迟高）
❌ 不推荐：需要装驱动的设备（树莓派不兼容）
```
...
| 摄像头麦克风 | `USB 摄像头麦克风 免驱` |
| 音箱 | `Y57 小扩音器`或`USB 小音箱 免驱` |
...
card 3: WebCamera     — USB 摄像头麦克风一体机（语音输入）
card 4: USB2.0 Device — Y57 小扩音器（语音输出）
```
